...
func TestWorkflow_OverallScoreValidation(t *testing.T) {
	if os.Getenv("GORGON_MUTANT_ID") != "" {
		t.Skip("skipping full pipeline test during mutation run")
	}
	...
	outputDir := filepath.Join(repoRoot, "tests/e2e/testdata/TestWorkflow_OverallScoreValidation/output")
...
func TestWorkflow_NoMutantsLost(t *testing.T) {
	...
	outputDir := filepath.Join(repoRoot, "tests/e2e/testdata/TestWorkflow_NoMutantsLost/output")
